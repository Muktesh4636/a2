...
def _parse_bankroll(raw, default):
...
def create_session(request):
    bankroll = _parse_bankroll(
        request.data.get("bankroll"), settings.STARTING_BANKROLL
    )
    session = GameSession.objects.create(bankroll=bankroll)
    return Response(
        {
...
@api_view(["POST"])
def sync_bankroll(request, session_id):
    session = get_object_or_404(GameSession, id=session_id)
    if "bankroll" not in request.data:
        return Response({"error": "bankroll is required"}, status=400)
    bankroll = _parse_bankroll(request.data.get("bankroll"), None)
    if bankroll is None:
        return Response({"error": "bankroll must be an integer"}, status=400)
    session.bankroll = bankroll
    session.save(update_fields=["bankroll", "updated_at"])
    return Response(
        {
