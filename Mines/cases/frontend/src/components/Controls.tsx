...
<label className="field">
      ...
      {lastStatus === 'lost' && (
        <div className="status-banner status-banner--lost">
          Opened {formatMultiplier(lastMultiplier ?? 0)} — bet reduced
        </div>
      )}
      ...
      <button
        ...
        onClick={onPlay}
      >
        {busy ? 'Opening…' : `Bet ${formatMoney(betAmount)}`}
      </button>

      <p className="hint">
        Place a bet — cases slide past the pin. When they stop, the case opens
        and pays that multiplier.
      </p>
    </aside>
  )
}
