...
TWO = Decimal('0.01')
FOUR = Decimal('0.0001')
POOL: list[dict[str, Any]] = settings.SLIDE_POOL
REEL_LENGTH = settings.SLIDE_REEL_LENGTH
WIN_INDEX_MIN = settings.SLIDE_WIN_INDEX_MIN
WIN_INDEX_MAX = settings.SLIDE_WIN_INDEX_MAX
...
def get_or_create_player(player_id: str | None) -> Player:
    if player_id:
        try:
            ...
            pass
    return Player.objects.create(balance=money(settings.SLIDE_STARTING_BALANCE))
...
def play_round(player: Player, bet_amount: Decimal) -> Game:
    min_bet = money(settings.SLIDE_MIN_BET)
    max_bet = money(settings.SLIDE_MAX_BET)
    ...
    # Very low multipliers can still be "won" — lost only if mult is 0 (unused)
    status = Game.Status.WON if multiplier > 0 else Game.Status.LOST
