"""
VIP Vortex — luxury table rules (higher stakes than Vortex 2).

  Water (blue):  2X → 6X → 12X
  Earth (green): 3X → 10X → 22X → 40X → 70X
  Fire (red):    5X → 20X → 45X → 80X → 140X → 250X → 400X

  Bonus ceilings: Water 25X · Earth 100X · Fire 999X
"""
...
def part_amount(fill: dict, bet: Decimal) -> Decimal:
    total = Decimal("0")
...
def session_snapshot(session, *, message: str | None = None, drop: str | None = None, extra: dict | None = None) -> dict:
    fill = {
        "water": session.water,
        "earth": session.earth,
        "fire": session.fire,
    ...
    payload = {
        ...
        "payout": float(money(bet * Decimal(str(mult)))),
        "can_part": can_part(fill),
        "part_amount": float(part_amount(fill, bet)) if can_part(fill) else 0.0,
