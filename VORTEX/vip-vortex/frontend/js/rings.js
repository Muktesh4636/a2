...
export function createRingBoard(canvas) {
  ...
  function drawBonusBadge(key, ring) {
    ...
    if (asset) {
      const s = iconR * 2.2;
      ctx.drawImage(asset, iconX - s / 2, -s / 2, s, s);
    } else {
      ctx.font = "14px serif";
  ...
  function draw3DRingTrack(radius, dark, thick) {
    ...
    // VIP bronze / gold pipe layers
    const layers = dark
      ? [
          { o: 0.48, w: 0.22, c: "#3a2a18" },
          { o: 0.32, w: 0.28, c: "#4a3620" },
          { o: 0.12, w: 0.34, c: "#5c4328" },
          { o: 0.0, w: 0.42, c: "#6b5130" },
          { o: -0.12, w: 0.34, c: "#5c4328" },
          { o: -0.28, w: 0.28, c: "#4a3620" },
          { o: -0.44, w: 0.2, c: "#3a2a18" },
        ]
      : [
          { o: 0.48, w: 0.22, c: "#8a6a3a" },
          { o: 0.34, w: 0.3, c: "#a88445" },
          { o: 0.18, w: 0.36, c: "#c4a05a" },
          { o: 0.02, w: 0.44, c: "#d8b86a" },
          { o: -0.12, w: 0.34, c: "#c4a05a" },
          { o: -0.28, w: 0.3, c: "#a88445" },
          { o: -0.44, w: 0.22, c: "#8a6a3a" },
        ];
  ...
  function drawCenterWell(dark) {
    withDepth("center", (_x, _y, d) => {
      ctx.beginPath();
      ctx.arc(0, d.thick + 3, 78, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(0,0,0,0.14)";
      ctx.fill();

      const well = ctx.createRadialGradient(-18, -22, 6, 0, 0, 76);
  ...
  function drawBoard() {
    ...
    drawGreyValley(RINGS.earth.radius + half, RINGS.fire.radius - half, "earth", "fire", dark, {
      light: "#1a1208", dark: "#0d0a06",
    });
    drawGreyValley(RINGS.water.radius + half, RINGS.earth.radius - half, "water", "earth", dark, {
      light: "#140e08", dark: "#0a0704",
    });
    drawGreyValley(78, RINGS.water.radius - half, "center", "water", dark, {
      light: "#1a1208", dark: "#0d0a06",
    });
