...
function resultLabel(side) {
  return side === "mirror" ? "MIRROR" : "CLASH";
}
...
function updateUI() {
  ...
  else if (!state.side) els.roundInfo.textContent = "Pick Mirror or Clash";
...
async function deal() {
  ...
  const colorWord = result.matched_color ? "MATCH" : "SPLIT";
  els.sumValue.textContent = colorWord;
  ...
  if (result.won) {
    ...
    els.roundInfo.textContent = `${resultLabel(result.result_side)} · win ₹${result.payout}`;
  } else {
    els.outcome.textContent = `${resultLabel(result.result_side)} hits · you lose`;
    els.outcome.classList.add("lose");
    els.roundInfo.textContent = `${resultLabel(result.result_side)} hits`;
  }
