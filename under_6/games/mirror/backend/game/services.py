"""Mirror — second card must match (or clash) the first card's color."""
...
def result_side(card1: Card, card2: Card) -> str:
    return "mirror" if card1.red == card2.red else "clash"
...
def deal_round(bet_side: str, chip: int, bankroll: int) -> dict:
    ...
    if bet_side not in settings.PAYOUTS:
        raise ValueError("Invalid bet side. Use mirror or clash.")
    ...
    card1 = pick_card()
    card2 = pick_card()
    won_side = result_side(card1, card2)
