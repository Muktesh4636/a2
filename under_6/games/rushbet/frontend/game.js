...
const SESSION_KEY = "rushbet_session_id";
...
function updateUI() {
  ...
  if (state.busy) els.roundInfo.textContent = "Rushing…";
  else if (!state.side) els.roundInfo.textContent = "Pick Low or High";
  else els.roundInfo.textContent = `₹${state.chip} on ${resultLabel(state.side)} — rush when ready`;
}
...
async function deal() {
  ...
  els.roundInfo.textContent = "Dropping…";
  await dealIn(els.rig1);
  await sleep(40);
  await dealIn(els.rig2);
  await sleep(80);

  els.roundInfo.textContent = "Flip!";
