...
function renderBanners() {
  ...
  slides.forEach((game, i) => {
    ...
    copy.innerHTML = `
      ...
      <h3>${game.title}</h3>
      <p>Tap Play to open — long-press tiles below to preview</p>
    `;
  ...
  window.setInterval(() => {
    ...
    auto = (auto + 1) % slides.length;
    track.scrollTo({ left: track.clientWidth * auto, behavior: "smooth" });
  }, 4500);
}
...
function renderDesktopNav() {
  ...
  if (!list) return;
  list.innerHTML = "";
  const items = [{ id: "all", title: "All Games" }, ...CATEGORIES];
  items.forEach((item) => {
    const btn = document.createElement("button");
    btn.type = "button";
