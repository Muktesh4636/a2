(function () {
  ...
  const ROWS = 3;
  const START = 1000;
  const KEY = "midnight-manor-balance";
  const BREW_KEY = "midnight-manor-brew";
  ...
  const SYMBOLS = [
    { id: "ghost", wild: true, weight: 3, pays: [0, 0, 12, 60, 250] },
    { id: "pumpkin", weight: 5, pays: [0, 0, 8, 35, 120] },
    { id: "cat", weight: 6, pays: [0, 0, 6, 25, 90] },
    { id: "hat", weight: 7, pays: [0, 0, 5, 20, 70] },
    { id: "potion", potion: true, weight: 5, pays: [0, 0, 4, 15, 50] },
    { id: "skeleton", weight: 8, pays: [0, 0, 4, 12, 40] },
    { id: "candy", weight: 12, pays: [0, 0, 3, 8, 25] },
    { id: "spellbook", scatter: true, weight: 4, pays: [0, 0, 2, 10, 40] },
  ];
  ...
  const els = {
    ...
    brewMult: document.getElementById("brewMult"),
  ...
  let brew = Number(localStorage.getItem(BREW_KEY)) || 0;
  let betIndex = 2;
  let spinning = false;
  let auto = false;
  let freeSpins = 0;
  ...
  function save() {
    ...
    localStorage.setItem(BREW_KEY, String(brew));
  }
  function pick() { return bag[Math.floor(Math.random() * bag.length)]; }
  function wait(ms) { return new Promise((r) => setTimeout(r, ms)); }
  function brewMultiplier() { return 1 + brew; }
  ...
  function buildVials() {
    els.vials.innerHTML = "";
    for (let i = 0; i < MAX_BREW; i++) {
      const v = document.createElement("div");
      v.className = "vial" + (i < brew ? " filled" : "");
      els.vials.appendChild(v);
    }
  }
  ...
  function updateHud() {
    ...
    els.brewMult.textContent = `x${brewMultiplier()}`;
    buildVials();
  ...
  function collectPotions() {
    ...
    for (let c = 0; c < COLS; c++) {
      for (let r = 0; r < ROWS; r++) {
        if (byId[grid[c][r]].potion) n += 1;
      }
    }
    brew = Math.min(MAX_BREW, brew + n);
  }

  function linePay(line) {
    const ids = line.map((row, col) => grid[col][row]);
    let base = ids.find((id) => !byId[id].wild && !byId[id].scatter && !byId[id].potion);
    ...
    return {
      win: mult * BETS[betIndex] * brewMultiplier(),
  ...
  function evaluate() {
    ...
    if (books >= 3) {
      total += (byId.spellbook.pays[books - 1] || 0) * BETS[betIndex] * brewMultiplier();
      bookCells.forEach((k) => winCells.add(k));
  ...
  async function spinOnce() {
    ...
    if (total > 0) {
      ...
      // consume brew after a paying spin
      if (brew > 0) brew = Math.max(0, brew - 1);
    }
    save();
    setBusy(false);
    updateHud();
