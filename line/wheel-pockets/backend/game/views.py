...
@require_GET
def config_view(request): return JsonResponse({'currency': 'INR'})
...
def play_view(request):
    ...
    try:
        with transaction.atomic():
            ...
            play = Play.objects.create(
                player=p, bet_amount=bet, pocket_id=result['pocket_id'],
    ...
    return JsonResponse({
        'play_id': str(play.id), 'pocket_id': result['pocket_id'], 'color': result['color'],
