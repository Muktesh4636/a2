...
/** Deep Dive flight arc — seabed rise toward surface */
const P0 = { x: 2, y: 82 }
const P1 = { x: 42, y: 70 }
const P2 = { x: 94, y: 14 }
const BOTTOM_Y = 100
...
<div class="stage-wrap" id="stageWrap">
  ...
  <div class="depth-fog" aria-hidden="true"></div>
  <div class="stage-logo">Deep Dive</div>
  <svg class="flight-svg" viewBox="0 0 ${VIEW_W} ${VIEW_H}" preserveAspectRatio="none" aria-hidden="true">
    <defs>
      <linearGradient id="trailGrad" x1="0%" y1="0%" x2="0%" y2="100%">
        <stop offset="0%" style="stop-color:#7dd3fc;stop-opacity:0.35" />
        <stop offset="45%" style="stop-color:#22d3ee;stop-opacity:0.18" />
        <stop offset="100%" style="stop-color:#082f49;stop-opacity:0.08" />
      </linearGradient>
      <linearGradient id="lineGrad" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%" style="stop-color:#67e8f9;stop-opacity:0.35" />
        <stop offset="50%" style="stop-color:#a5f3fc;stop-opacity:0.75" />
        <stop offset="100%" style="stop-color:#ecfeff;stop-opacity:0.95" />
      </linearGradient>
    </defs>
    <path id="trailFill" class="trail-fill" d="" />
    <path id="trailLine" class="trail-line" d="" />
  </svg>
  <img id="jetFly" class="jet-fly" src="${ASSET}submarine.svg" alt="" width="140" draggable="false" />
  <div class="multiplier-layer">
    <div class="status-line" id="statusLine">Pressurizing</div>
...
<p class="demo-note">Deep Dive demo — play money only. Same crash loop as Aviator / Jet / Maestro.</p>
`
...
function setJet(progress: number, now: number, flying: boolean) {
  ...
  const marginX = (140 / stageW) * VIEW_W * 0.48
  const marginY = (70 / stageH) * VIEW_H * 0.48
...
function beginWaiting(now: number) {
  ...
  statusLine.textContent = 'Pressurizing'
...
function beginCrash(now: number) {
  ...
  statusLine.textContent = 'Hull breach!'
  statusLine.classList.remove('hidden')
  finalizeRoundBets()
  updateAllPanels()
